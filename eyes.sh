...
while true
do
	sleep "$rest_time" && zenity --question --text "Your eyes are tired. Rest?" --title="Eye Care" --cancel-label="Snooze" 2> /dev/null

	if [ "$?" -eq 0 ] # 0 means user pressed 'yes' button
	then
		show_progress_bar
	else 					#Snooze for 5 minutes
		zenity --notification --text "Reminder snoozed for 5 minutes." 2> /dev/null
		sleep "$snooze_time" && zenity --question --text "Snooze period over. Your eyes are still tired. Rest?" --title="Eye Care" --cancel-label="Skip" 2> /dev/null
		if [ "$?" -eq 0 ] # 0 means user pressed 'yes' button
		then
			show_progress_bar
		else
			zenity --notification --text "A reminder has been skipped." 2> /dev/null
		fi
	fi
done
